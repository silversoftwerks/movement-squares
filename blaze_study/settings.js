...
function loadSettings() {
    ...
    if (settingsJson) {
        try {
            ...
            if (!ringCountControl || !segmentCountControl) {
                console.error("Critical controls not found. Cannot load settings.");
                return false;
            }
            ...
            if (settings.segmentCount && segmentCountControl) segmentCountControl.value = settings.segmentCount;
            if (settings.stripeCount && stripeCountControl) stripeCountControl.value = settings.stripeCount;
            if (settings.angleOffset && angleOffsetControl) angleOffsetControl.value = settings.angleOffset;
            if (settings.rotationSpeed && rotationSpeedControl) rotationSpeedControl.value = settings.rotationSpeed;
            if (settings.alternateRotation !== undefined && alternateRotationControl) alternateRotationControl.checked = settings.alternateRotation;
            if (settings.colorScheme && colorSchemeControl) colorSchemeControl.value = settings.colorScheme;
            if (settings.stripAngle && stripAngleControl) stripAngleControl.value = settings.stripAngle;
            if (settings.alternateStripAngles !== undefined && alternateStripAnglesControl) alternateStripAnglesControl.checked = settings.alternateStripAngles;
            if (settings.ringWidth && ringWidthControl) ringWidthControl.value = settings.ringWidth;
            if (settings.gradientEnabled !== undefined && gradientEnabledControl) gradientEnabledControl.checked = settings.gradientEnabled;
            if (settings.gradientIntensity && gradientIntensityControl) gradientIntensityControl.value = settings.gradientIntensity;
            if (settings.edgeBrightness && edgeBrightnessControl) edgeBrightnessControl.value = settings.edgeBrightness;
            if (settings.centerDarkness && centerDarknessControl) centerDarknessControl.value = settings.centerDarkness;
            if (settings.gradientWidth && gradientWidthControl) gradientWidthControl.value = settings.gradientWidth;
            if (settings.gradientCurve && gradientCurveControl) gradientCurveControl.value = settings.gradientCurve;
            if (settings.primaryColor && primaryColorControl) primaryColorControl.value = settings.primaryColor;
            if (settings.secondaryColor && secondaryColorControl) secondaryColorControl.value = settings.secondaryColor;
            if (settings.glowEnabled !== undefined && glowEnabledControl) glowEnabledControl.checked = settings.glowEnabled;
            if (settings.glowIntensity && glowIntensityControl) glowIntensityControl.value = settings.glowIntensity;
            if (settings.glowSize && glowSizeControl) glowSizeControl.value = settings.glowSize;
            if (settings.glowColor && glowColorControl) glowColorControl.value = settings.glowColor;
            
            // Update value displays
            if (typeof updateValueDisplays === 'function') {
                updateValueDisplays();
            }
...
function applyRingSettings() {
    ...
    // Clear the stored settings to prevent reapplication
    // window.storedRingSettings = null;
}
